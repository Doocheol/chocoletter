...
const MainYourBeforeView: React.FC = () => {
  ...
  const [isGiftBoxNameLoaded, setIsGiftBoxNameLoaded] = useState<boolean>(false);
  ...
  const tutorialIcons = useMemo(
    () => [tutorialIconRef, dummyRef, dummyRef, dummyRef, dummyRef, dummyRef, dummyRef, giftBoxRef],
    []
  );
    ...
    if (giftBoxId) {
      getGiftBoxName(giftBoxId)
        .then((data) => {
          ...
          const validName = name && name.trim() !== "" ? name.trim() : null;
          ...
          if (validName) {
            ...
            if (storedName && storedName.trim() !== "") {
              ...
              localStorage.setItem("giftBoxName", DEFAULT_GIFTBOX_NAME);
            }
          }
      ...
      <div className="w-full max-w-sm min-h-screen h-[calc(var(--vh)*100)] flex flex-col bg-gradient-to-b from-[#E6F5FF] to-[#F4D3FF]">
        ...
        >
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <div>
              <span className="text-2xl text-center max-w-full truncate">{recipientNickname}</span>
              <span className="text-lg text-center max-w-full truncate"> 님의 </span>
            </div>
            <span className="text-lg text-center max-w-full truncate">초콜릿 보관함</span>
          </div>
        </div>
