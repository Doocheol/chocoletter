...
const ChatRoomListView = () => {
		...
		const loadLastMessages = async () => {
			try {
				...
				setChatRooms((prevRooms) => {
					return JSON.stringify(prevRooms) !==
	...
	const handleRoomClick = (room: ChatRoom) => {
		navigate(`/chat/room/${room.roomId}`, {
			state: { roomId: room.roomId, nickName: room.nickName },
		});
	};
						...
						>
							...
							<div className="flex flex-col">
								<div className="w-full h-full">
									...
									<p className="font-[Pretendard] text-[12px] text-[#696A73] max-w-full overflow-hidden whitespace-nowrap text-ellipsis">
										{room.lastMessage ? (
											room.lastMessage
										) : (
