...
const TestVideoRoomView = () => {
	...
	const muteOrNotHandler = () => {
		videoState.publisher?.publishAudio(
			!videoState.publisher.stream.audioActive
		);
		setIsAudio((prev) => !prev);
	};

	const videoOffOrNotHandler = () => {
		videoState.publisher?.publishVideo(
			!videoState.publisher.stream.videoActive
		);
	...
	return (
		<div key={refreshKey}>
			<div className="w-full min-h-screen flex flex-col justify-center items-center bg-[#A8A8A8] relative overflow-hidden">
				{isItThere && countFive ? (
					<FiveSecondModal leftTime={leftTime} />
				) : null}
				{isTerminate && (
				...
				>
					{videoState.publisher && (
						<video
							...
							ref={(el) =>
								el && videoState.publisher?.addVideoElement(el)
							}
						/>
					)}
				</div>
				...
				<div className="absolute top-9 px-[15px] py-[5px] bg-chocoletterGiftBoxBg rounded-[17px] justify-center items-center gap-[9px] inline-flex z-20">
					...
					<div
						className={`text-center ${
							leftTime <= 5
								? "text-chocoletterWarning"
								: "text-chocoletterPurpleBold"
						} text-2xl font-normal font-sans leading-snug`}
					>
						{leftTime}
					</div>
				</div>
				...
				<div className="flex w-full bottom-[5dvh] justify-center gap-x-7 items-center absolute pt-3">
					<div
						className={`w-[9dvh] h-[9dvh] ${
							isAudio ? "bg-white" : "bg-black"
						} rounded-[100px] justify-center items-center gap-2.5 inline-flex z-20`}
					>
						<button
							onClick={muteOrNotHandler}
					...
					<div
						className={`w-[9dvh] h-[9dvh] ${
							isVideo ? "bg-white" : "bg-black"
						} rounded-[100px] justify-center items-center gap-2.5 inline-flex z-20`}
					>
						<button
							onClick={videoOffOrNotHandler}
							className="w-full h-full aspect-square flex justify-center items-center"
						>
							{isVideo ? (
