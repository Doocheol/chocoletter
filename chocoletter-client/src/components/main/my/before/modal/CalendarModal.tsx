  ...
  return (
    ...
    >
      <div className="flex flex-col justify-center items-center w-full">
        {/* 헤더 영역 */}
        <img src={calendar_icon} className="w-6 h-6 mb-2 ml-4" />

        <div className="w-full text-xl font-normal px-24 mb-4 whitespace-nowrap">
          <div className="">발렌타인데이 일정</div>
        </div>
                ...
                return (
                  <div
                    ...
                    className={`
                      ...
                      ${
                        block.isScheduled
                          ? "bg-chocoletterPurple bg-opacity-60 shadow-sm rounded-sm"
                          : "bg-white"
                      }
                    `}
                  >
              ...
              <div className="flex flex-col gap-2 ml-4">
                {sortedSchedules.length > 0 ? (
                  sortedSchedules.map((item, i) => (
                    <div
                      key={`${item.start}-${i}`}
                      className="text-xs bg-chocoletterPurpleBold bg-opacity-60 text-white rounded-xl shadow-s text-center"
                    >
                      {item.start}
                      {/* 종료 시각은 보여주지 않음 */}
                    </div>
                  ...
                  <div className="text-gray-300 text-sm">일정이 없어요!</div>
                )}
              </div>
            </div>
          </div>
