...
export const CheckLetterUseButton = ({
	onClick,
}: CheckLetterUseButtonProps) => {
	...
	return (
		<>
			<EmergeProblemModal
				isOpen={isPropblemOpen}
				onClose={allModalCloseHandler}
			/>
			<PurpleButton
				onClick={checkLetterClickHandler}
				className="text-white bg-chocoletterPurpleBold border border-black"
			>
				사용하기
			</PurpleButton>
		</>
