...
```
- Sentry는 애플리케이션에서 발생하는 오류를 감지하고 추적하는 실시간 모니터링 도구임. 다양한 프로그래밍 언어와 프레임워크를 지원하며, 개발자가 코드의 버그를 신속하게 파악하고 해결할 수 있도록 도와줌.

- 에러가 발생하면 Sentry는 자동으로 로그를 수집하고, 오류가 발생한 코드 라인과 실행 당시의 상태를 분석하여 상세한 리포트를 제공함. 또한, 동일하거나 유사한 오류를 그룹화하여 관리할 수 있어, 개발자가 반복적으로 발생하는 문제를 효율적으로 해결할 수 있도록 해줌.

- Slack, Jira, GitHub 등과 연동할 수 있어, 오류 발생 시 즉각적인 알림을 받을 수 있으며, 팀원들과 공유하여 협업할 수도 있음. 특히 프론트엔드 개발에서는 사용자 환경에서 발생하는 오류를 효과적으로 추적하고 디버깅하는 데 유용함.

- 또한, Breadcrumbs 기능을 제공하여 오류 발생 전후의 사용자 행동을 분석할 수 있도록 하며, 성능 모니터링 기능을 통해 애플리케이션의 속도와 안정성을 평가하는 데에도 활용됨.
```

### 예제 코드

#### 1. **JavaScript(React)에서 Sentry 설정**
...
Sentry.init({
    dsn: "https://your-dsn-url@sentry.io/your-project-id",
    integrations: [new BrowserTracing()],
    tracesSampleRate: 1.0, // 성능 모니터링을 위한 샘플링 비율 (1.0은 100% 수집)
});
...
이 코드를 사용하면 `handleError` 버튼을 클릭할 때 Sentry가 오류를 감지하고 로그를 기록함.

---

#### 2. **Node.js(Express)에서 Sentry 설정**
...
이렇게 설정하면 서버에서 오류가 발생할 경우 Sentry가 자동으로 감지하고 기록함.

---

#### 3. **Python(Flask)에서 Sentry 설정**
...
이 코드에서는 Flask 애플리케이션에서 발생하는 오류를 Sentry가 감지하여 기록함.
...
유닛 테스트는 개별 함수나 컴포넌트의 동작을 검증하는 테스트임.  
Jest와 React Testing Library를 사용한 예제임.
...
test("두 숫자를 더한 값을 반환해야 함", () => {
    expect(sum(2, 3)).toBe(5);
});
```

React 컴포넌트의 유닛 테스트 예제:
...
```javascript
// Button.test.js (유닛 테스트)
import { render, screen, fireEvent } from "@testing-library/react";
import Button from "./Button";

test("버튼 클릭 시 콜백 함수가 실행돼야 함", () => {
...
통합 테스트는 여러 모듈이 상호작용할 때 정상적으로 동작하는지 확인하는 테스트임.  
예제는 API 요청을 포함한 React 컴포넌트 테스트임.

```javascript
// fetchData.js (API 호출 함수)
...
```javascript
// FetchComponent.test.js (통합 테스트)
import { render, screen, waitFor } from "@testing-library/react";
import FetchComponent from "./FetchComponent";
import { fetchData } from "./fetchData";

jest.mock("./fetchData"); // API 호출을 Mocking하여 실제 네트워크 요청 없이 테스트 수행

test("API 데이터를 정상적으로 렌더링해야 함", async () => {
    fetchData.mockResolvedValue({ title: "테스트 데이터" });

    render(<FetchComponent />);

    const element = await waitFor(() => screen.getByText("테스트 데이터"));
    expect(element).toBeInTheDocument();
});
```

**Mock API란?**  
Mock API는 테스트 환경에서 실제 API를 호출하지 않고, 예상된 응답을 반환하도록 설정하는 방식임.  
이를 통해 네트워크 상태에 의존하지 않고 안정적으로 테스트할 수 있으며, API 서버가 개발되지 않았거나 변경될 경우에도 테스트를 지속할 수 있음.  
위 코드에서는 `jest.mock("./fetchData")`를 사용하여 실제 API 요청 없이 `mockResolvedValue`를 통해 가짜 데이터를 반환하도록 함.

---

### 3. **E2E 테스트(End-to-End Test)**

E2E 테스트는 사용자의 실제 사용 흐름을 테스트함.  
Cypress를 사용한 로그인 시나리오 예제임.

```javascript
// cypress/integration/login.spec.js
describe("로그인 테스트", () => {
    it("올바른 정보를 입력하면 로그인에 성공해야 함", () => {
        cy.visit("/login"); // 로그인 페이지로 이동

        cy.get("input[name='username']").type("testuser"); // 사용자 입력
        cy.get("input[name='password']").type("password123");
        cy.get("button[type='submit']").click(); // 로그인 버튼 클릭

        cy.url().should("include", "/dashboard"); // 로그인 성공 후 URL 확인
    });

    it("잘못된 정보를 입력하면 오류 메시지가 표시돼야 함", () => {
        cy.visit("/login");
        ...
        cy.contains("로그인 실패").should("be.visible"); // 오류 메시지 확인
    });
});
```
...
| 테스트 유형     | 목적                                           | 예제 코드                       |
| --------------- | ---------------------------------------------- | ------------------------------- |
| **유닛 테스트** | 개별 함수나 컴포넌트 테스트                    | `sum.test.js`, `Button.test.js` |
| **통합 테스트** | 여러 모듈(API 포함)이 올바르게 동작하는지 검증 | `FetchComponent.test.js`        |
| **E2E 테스트**  | 사용자의 실제 사용 흐름 테스트                 | `login.spec.js`                 |

각 테스트 유형을 적절히 조합하면 애플리케이션의 신뢰성을 높일 수 있음.  
Mock API를 활용하면 네트워크 요청 없이도 API 테스트가 가능하여 보다 안정적인 테스트 환경을 구축할 수 있음.
...
-   [FIX] 채팅방 읽음처리
-   [FIX] 채팅방 목록 - 마지막메세지, 안읽은 메세지 수

### ❣️ 오늘 배운 점

### | 입력창 fixed vs footer 비교
...
| ----------------------------------------------------------- | -------------------------------------------------------- | ------------------------------------------------------------------- |
| **✅ `fixed` (현재 코드처럼 사용)**                         |                                                          |                                                                     |
| 📌 항상 화면 하단에 고정됨                                  | ✅ **입력창이 항상 보임** (스크롤 내려도 유지)           | ❌ 키보드가 열릴 때 가려질 수 있음 (모바일 환경에서 추가 조정 필요) |
| 📌 채팅방 UI에서 많이 사용됨 (카카오톡, 페이스북 메신저 등) | ✅ 채팅방처럼 **입력창이 항상 보이는 UX** 제공           | ❌ iOS에서 `fixed` 요소가 예상과 다르게 동작할 수도 있음            |
| **✅ `footer` (HTML `<footer>` 사용)**                      |                                                          |                                                                     |
| 📌 레이아웃 안에서 자연스럽게 위치                          | ✅ 일반적인 페이지 구조와 잘 맞음                        | ❌ 스크롤을 내리면 입력창이 보이지 않음                             |
| 📌 키보드가 열릴 때 자동으로 푸시됨                         | ✅ 키보드가 자동으로 밀어 올려주는 효과 발생 (일부 환경) | ❌ 채팅 UI에서는 비효율적 (입력하려면 스크롤해야 함)                |

---

### **🚀 언제 `fixed` vs `footer`를 사용해야 할까?**

#### **✅ `fixed`를 사용해야 하는 경우**

✔ **채팅 입력창 (카카오톡, 페이스북 메신저 스타일)**  
✔ **입력창이 항상 화면 하단에 유지되어야 할 때**  
✔ **스크롤을 내려도 입력창이 보이게 하고 싶을 때**

📌 **카카오톡, 페이스북 메신저, 인스타그램 DM 같은 채팅앱이 이 방식을 사용**

---

#### **✅ `footer`를 사용해야 하는 경우**

✔ **일반적인 폼 입력 (로그인, 검색창, 게시글 작성 등)**  
✔ **입력창이 특정 레이아웃 안에 포함되어야 할 때**  
✔ **스크롤 시 입력창이 자연스럽게 사라져도 되는 경우**

📌 **뉴스 댓글 입력창, 블로그 작성, 일반적인 입력 필드에서 많이 사용**

---

### **📌 결론: 채팅 입력창은 `fixed`가 더 적합!**

현재 사용 중인 `fixed`가 **카카오톡 같은 채팅 UI**에서는 더 적합합니다.  
하지만, **모바일 환경에서 키보드가 가리는 문제를 해결하려면 추가 조정이 필요**합니다.
...
-   개발 전에 테스트 코드를 먼저 작성하고, 이후 실제 코드를 구현하는 방식.
-   **단계**:
    1.  실패하는 테스트 작성
...
### 2. **BDD (Behavior-Driven Development, 행동 주도 개발)**

-   TDD에서 확장된 개념으로, 요구사항을 행동 기반으로 정의한 후 테스트하는 방식.
-   **특징**:
    -   자연어(Gherkin)를 사용하여 테스트 시나리오 작성 (Given-When-Then 패턴)
    -   개발자, QA, 기획자가 협업하여 테스트 정의
-   **예시 (Gherkin 문법)**:
    ```gherkin
    ...
    ```
-   **대표적인 프레임워크**:
    -   Cucumber (Java, Ruby 등)
    -   SpecFlow (.NET)

---

### 3. **ATDD (Acceptance Test-Driven Development, 인수 테스트 주도 개발)**

-   TDD와 비슷하지만, 사용자 요구사항(인수 기준)을 기반으로 테스트를 먼저 작성.
-   고객/기획자가 직접 테스트 시나리오를 정의하고 개발자가 이를 만족하도록 개발.
-   **장점**:
    -   실제 사용자 요구를 충족하는지 검증 가능
    -   개발과 테스트가 동시에 진행됨
...
-   코드의 작은 단위(함수, 클래스 등)를 개별적으로 테스트하는 방식.
-   **예시 프레임워크**:
    -   JUnit (Java)
    -   PyTest (Python)
    -   Jest (JavaScript)
...
### 6. **System Test (시스템 테스트)**

-   전체 시스템이 예상대로 작동하는지 검증하는 테스트.
-   **유형**:
    -   기능 테스트 (Functional Testing)
    -   비기능 테스트 (Non-Functional Testing, 성능·보안 등 포함)

---

### 7. **Regression Test (회귀 테스트)**

-   코드 수정 후 기존 기능이 정상 동작하는지 검증하는 테스트.
-   **자동화 도구**:
    -   Selenium
    -   Cypress
...
-   시스템의 속도, 반응성, 안정성을 검증하는 테스트.
-   **유형**:
    -   부하 테스트 (Load Testing) – 사용량 증가 시 성능 확인
    -   스트레스 테스트 (Stress Testing) – 한계를 초과한 부하에서의 성능 확인
    -   지구력 테스트 (Endurance Testing) – 장시간 실행 시 성능 확인
-   **도구**:
    -   JMeter
    -   Locust
...
### 9. **Security Test (보안 테스트)**

-   시스템이 보안 취약점 없이 안전한지 검증하는 테스트.
-   **유형**:
...
### 10. **Usability Test (사용성 테스트)**

-   사용자 관점에서 시스템이 직관적으로 사용 가능한지 평가하는 테스트.

---

### 11. **Smoke Test (스모크 테스트)**

-   빌드가 정상적으로 동작하는지 빠르게 확인하는 최소한의 테스트.

---

### 12. **Sanity Test (건전성 테스트)**

-   특정 기능이 정상적으로 수정되었는지 확인하는 간단한 테스트.

---

### 13. **Exploratory Test (탐색적 테스트)**

-   정해진 테스트 케이스 없이 경험 기반으로 시스템을 탐색하며 발견하는 테스트.

---

이 외에도 다양한 테스트 방식이 존재하지만, TDD와 관련된 개념을 중심으로 정리해 보았습니다. 원하는 테스트 유형이 추가로 있으면 말씀해주세요! 😊
...
-   입력창에서 **마지막 글자가 반복되는 이유**는 **IME(입력기, 특히 한글 조합기)** 때문임.
-   한글과 같은 조합형 문자를 입력할 때, `onChange`와 `onCompositionEnd` 이벤트가 상호 작용하면서 **중복 입력**이 발생할 수 있음.

#### **⚠️ 문제 발생 과정**

1. 사용자가 **한글을 입력**할 때, `onCompositionStart`가 호출되어 **조합 상태**로 전환됩니다.
2. **조합이 끝나기 전에 `onChange`가 먼저 호출**되면서, 현재 조합 중인 글자가 상태(`message`)에 저장됩니다.
3. **조합 완료 후 `onCompositionEnd`가 호출되며, 최종 글자가 다시 상태(`message`)에 저장**됩니다.
4. **결과적으로 마지막 글자가 두 번 저장**되어 입력창에 반복되는 현상이 발생합니다.

---

### **🛠 해결 방법**

#### **✅ 1. `onCompositionStart`와 `onCompositionEnd`를 활용한 상태 관리**

IME 조합 중인지(`isComposing`) 여부를 추적하여, 조합 중일 때는 `onChange`에서 상태를 업데이트하지 않고, 조합이 끝난 후(`onCompositionEnd`) 상태를 갱신

**🔹 해결 코드**

```tsx
const [message, setMessage] = useState(""); // 입력값 상태
...
const handleCompositionEnd = (
    ...
    setMessage(e.currentTarget.value); // 조합 완료 후 최종 입력값 저장
};
...
✔ **조합 중일 때 (`isComposing === true`)는 `onChange`에서 `setMessage`를 호출하지 않음**  
✔ **조합이 완료되면(`onCompositionEnd`) 최종 입력값을 `setMessage`로 업데이트**  
👉 **이 방법을 적용하면 한글 입력 시 마지막 글자가 반복되지 않음!**

---

#### **✅ 2. `useRef`를 활용하여 중복 입력 방지**

입력값을 직접 상태로 관리하지 않고, **`useRef`를 사용하여 중복 입력을 방지**할 수도 있음.
...
const handleCompositionEnd = (
    ...
    messageRef.current = e.currentTarget.value; // 조합 완료 후 업데이트
};

const handleChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    if (!isComposing) {
        messageRef.current = e.target.value; // 중복 방지
    }
};
...
✔ `useRef`를 활용하여 직접 값을 조작하면, `useState`의 불필요한 업데이트가 발생하지 않음  
✔ 한글 조합이 끝난 후에만 값이 변경되므로 **중복 입력이 발생하지 않음**

---

### **🚀 최종 결론**

입력창에서 마지막 글자가 반복되는 이유는 **IME(입력기)의 조합 상태와 React의 상태 업데이트가 충돌**하기 때문.

✅ **해결 방법**

1. **IME 조합 중인지 (`isComposing`) 상태를 관리**하여, 조합 중에는 `onChange` 업데이트를 방지하고, `onCompositionEnd`에서만 최종 상태를 반영
2. **`useRef`를 사용하여 직접 값을 관리**하면 불필요한 상태 업데이트를 줄이고, 중복 입력을 방지할 수 있음
